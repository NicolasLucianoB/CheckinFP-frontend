...
export default function SignUpPage() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (result.message) {
          // Setar erro no campo email diretamente, sem trigger extra
    ...
    <main className="min-h-[calc(100vh-106px)] flex items-center justify-center bg-gray-100 p-4">
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-6 rounded shadow-md w-full max-w-sm space-y-4">
        ...
        <input
          ...
          placeholder="Função"
          className={`w-full border px-3 py-2 placeholder-gray-400 text-black rounded ${errors.role ? 'border-red-600' : 'border-gray-400'}`}
        />
        {typeof errors.role?.message === 'string' &&
          errors.role.message !== '*Vigia! todos os campos são obrigatórios' && (
